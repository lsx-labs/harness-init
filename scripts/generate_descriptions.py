...
# tight hook-style budget would falsely zero out counts on a large graph and miscategorize dirs.
GITNEXUS_QUERY_TIMEOUT = 30
...
PROVIDER_BY_CATEGORY = {
    ...
    "code_symbols": "ai_gitnexus",  # also generated by the AI path (no separate local summarizer)
    "test": "test_summary",
...
def gitnexus_query(cypher: str) -> list[list[str]]:
    try:
        r = subprocess.run(
            ...
            capture_output=True, text=True, timeout=GITNEXUS_QUERY_TIMEOUT)
    except (subprocess.TimeoutExpired, OSError):
        return []
    if r.returncode != 0:
        return []  # a non-zero exit means stderr is an error message, not a result table
    output = r.stdout.strip() or r.stderr.strip()  # GitNexus sometimes emits the table on stderr
    if not output:
        return []
...
def main():
    ...
    # Try AI + GitNexus first — reuse the evidence already gathered by the classification
    # report instead of re-collecting it (redundant GitNexus queries) per dir.
    ...
    if descriptions:
        ...
        if descriptions:
            ...
            # only finish if AI covered every dir; otherwise fall through so the dirs AI
            # missed still get a fallback (mirrors the deterministic block above)
            written_dirs = {change["dir"] for change in changes}
            dirs = [d for d in dirs if normalize_dir_key(d) not in written_dirs]
            if not dirs:
                quality_after = build_quality_report()
