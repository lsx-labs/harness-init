...
> **当前平台：Codex** — 主入口是 `AGENTS.md`；根 `CLAUDE.md` 也通过托管块保持可读，不做整文件同步。
> 子目录 `AGENTS.md` / `CLAUDE.md` 都维护 GitNexus facts block；Codex 运行时 `AGENTS.md` 优先。
...
子目录 `AGENTS.md` / `CLAUDE.md` 都只维护确定性 GitNexus facts block；不再 copy/sync 整文件，也不由 AI 自动写约束或危险操作 prose。`--platform codex` 只决定文件顺序：`AGENTS.md` 优先，`CLAUDE.md` 同步维护同一 facts block。
