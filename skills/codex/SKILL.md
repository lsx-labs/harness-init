...
|---|---|
| `skip` | 无操作 |
| `copy` | `python3 ~/.local/bin/sync-docs.py . --platform codex` |
...
**Copy**（脚本完成）：`python3 ~/.local/bin/sync-docs.py . --platform codex --dirs {copy列表}`
...
Layer 2: spawn 子 Agent 生成 src/utils/parser/AGENTS.md（并行）
  ↓ 等待完成
Layer 1: spawn 子 Agent 生成 src/utils/AGENTS.md（可复用下层）
```
