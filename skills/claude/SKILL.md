...
生成策略：
1. `CLAUDE.md` 已存在 → **跳过**
2. `CLAUDE.md` 不存在但 `AGENTS.md` 存在 → **复制 `AGENTS.md` 为 `CLAUDE.md`**（内容一致，跨平台复用）
3. 都不存在 → **从模板生成**

**同步规则**：更新 `CLAUDE.md` 后，如果 `AGENTS.md` 也存在，将更新内容同步复制过去（Hook 自动执行）。
...
**自底向上、逐层并行**：先找出所有需要生成的目录，按深度分层，从最深的叶子层开始，同层目录并行 spawn 子 Agent，等该层全部完成后再生成上一层（上层可复用下层总结）。

```
执行顺序（深度优先）：

Layer 2（最深叶子，并行）：
  src/core/engine/     ← 子 Agent A ─┐
  src/core/parser/     ← 子 Agent B  ├─ 并行
  src/api/handlers/    ← 子 Agent C  │
  src/api/middleware/   ← 子 Agent D ─┘
         ↓ 等待全部完成
Layer 1（中间层，并行，复用下层）：
  src/core/            ← 子 Agent E ─┐
  src/api/             ← 子 Agent F ─┘ 并行
         ↓ 等待全部完成
Layer 0（顶层，复用下层）：
  src/                 ← 主 Agent 生成
```
...
规则：
- **前置判断**：对方平台文件已存在（如 `*/AGENTS.md`）→ 直接复制为 `*/CLAUDE.md`，跳过 GitNexus 查询。只对需要从零生成的目录 spawn 子 Agent
- 同层所有目录并行 spawn 子 Agent（每个目录一个）
- 每层等待全部完成后才开始上一层
- 只有一层时直接全部并行，无需分层
- 子 Agent prompt 包含：目录路径、项目名、输出模板（见下方）
