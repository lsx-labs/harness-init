...
复杂模块需要独立的约束文件。Claude Code 进入子目录时自动加载，不在该目录工作时不消耗 context。

**触发条件**：CODE_MAP.md 中符号数 ≥ 100 的目录。

**生成流程**：
1. 从 CODE_MAP.md 筛出符号数 ≥ 100 的目录
2. 检查该目录是否已有 CLAUDE.md / AGENTS.md，已有则跳过
3. AI 读取该目录的 3-5 个核心源文件
4. 生成 CLAUDE.md 和 AGENTS.md（内容一致），写入该目录
...
{该模块的测试命令，如 pytest tests/autoresearch/distributed/ -v}

## 约束

{3-5 条该模块特有的编码约束，从代码结构和模式中提取}
...
控制：每个子目录文件 ≤ 30 行。只写 Claude 无法从代码推断的约束，不重复根 CLAUDE.md 已有的内容。

**与 CODE_MAP.md 的关系**：
- CODE_MAP.md = 全局导航索引（一行一个模块，回答"这里有什么"）
- 子目录 CLAUDE.md/AGENTS.md = 模块级深度约束（回答"在这里怎么干活"）
- 不重叠，互补
