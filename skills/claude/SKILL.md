...
> **当前平台：Claude Code** — 主入口是 `CLAUDE.md`；根 `AGENTS.md` 也通过托管块保持可读，不做整文件同步。
> 子目录 `CLAUDE.md` / `AGENTS.md` 都维护 GitNexus facts block；Claude 运行时 `CLAUDE.md` 优先。
...
子目录 `CLAUDE.md` / `AGENTS.md` 都只维护确定性 GitNexus facts block；不再 copy/sync 整文件，也不由 AI 自动写约束或危险操作 prose。`--platform claude` 只决定文件顺序：`CLAUDE.md` 优先，`AGENTS.md` 同步维护同一 facts block。

| action | 执行 |
|---|---|
| `refresh_facts` | `python3 ~/.local/share/harness-hooks/generate_subdir_harness.py . --refresh-facts --platform claude --dirs {目录列表}`；只替换 `<!-- harness:start/end -->` 内 facts |
