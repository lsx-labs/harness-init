...
2. 按目录深度排序（最深优先）
3. 对每个目录：
   - 不存在 → 全量生成（含 `<!-- auto:start/end -->` 标记）
   - 已存在 → **增量更新**：只重写 `<!-- auto:start -->` 到 `<!-- auto:end -->` 之间的内容，标记外的手动内容保留不动
...
**标记机制**：
- `<!-- auto:start -->` 到 `<!-- auto:end -->`：自动生成区域，增量更新时重写
